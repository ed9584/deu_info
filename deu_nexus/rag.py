...
def answer_with_rag(
    ...
    *,
    ...
    """최신 크롤 → 저장소 반영 → LangChain RAG 답변 + 출처."""
    ...
    if not arts:
        return {
            "reply": "불러온 공지가 없습니다. 설정·페이지 수를 확인해 주세요.",
            "sources": [],
            "crawl_summary": {
                "base": crawl.get("source", {}).get("base", ""),
                "mid": crawl.get("source", {}).get("mid", mid),
                "count_total": crawl.get("count_total", 0),
            },
            "ingest": None,
        }
    ...
    prompt = ChatPromptTemplate.from_template(
        "당신은 동의대학교 공지 내용을 근거로 답하는 도우미입니다.\n"
        "규칙:\n"
        "1) 아래 참고 문서에 없는 내용은 추측하지 말고 모른다고 합니다.\n"
        "2) 답변 마지막에 '출처'를 적고, 사용한 문서의 제목과 URL을 나열합니다.\n"
        "3) URL은 참고 문서에 있는 것만 사용합니다.\n\n"
        "참고 문서:\n{context}\n\n"
        "질문: {input}"
    )
